$(function() {
  // Custom sorting for status column: 'Active' > 'Offline'
  $.fn.dataTable.ext.order['status-active'] = function(settings, col) {
  ...
  const hotspotTable = $('#hotspotTable').DataTable({
    ...
    columnDefs: [
      { targets: 4, orderDataType: 'status-active' },
  ...
  // Add dropdown filter status
  const statusFilter = $('<select class="form-select form-select-sm ms-2" style="width:auto; display:inline-block;"><option value="">All Status</option><option value="Active">Active</option><option value="Offline">Offline</option></select>');
  ...
  statusFilter.on('change', function() {
    ...
    // Always sort Active status at top
    hotspotTable.order([4, 'desc'], [0, 'asc']).draw();
  });

  // Ensure status order is always priority during search
  $('#hotspotTable_filter input').on('input', function() {
    // Wait briefly for search to apply
    setTimeout(function() {
      hotspotTable.order([4, 'desc'], [0, 'asc']).draw();
    }, 100);
  });
  ...
  function updateActiveUserCount() {
    ...
    hotspotTable.rows({ search: 'applied' }).every(function() {
      ...
      if (data[4] && data[4].toLowerCase() === 'active') count++;
    });
...
function formatUptime(uptimeStr) {
    ...
    if (seconds) result += seconds[1] + ' seconds';
